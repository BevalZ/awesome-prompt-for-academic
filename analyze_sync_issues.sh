#!/bin/bash
#
# Comprehensive Prompt Synchronization Analysis
# Identifies and catalogs all data integrity and translation issues
#
...
REPORT_FILE="${SCRIPT_DIR}/SYNC_VALIDATION_REPORT.md"
...
echo -e "${BOLD}${BLUE}=== Prompt Synchronization Analysis ===${NC}\n"

echo -e "${BOLD}Discovering Issues...${NC}\n"

# Extract all categories
echo -e "${CYAN}1. Category Discovery${NC}"
echo "Categories on disk (EN/):"
ls "${PROMPTS_DIR}/EN/" | sed 's/.md//' | sed 's/^/   /' | sort

echo -e "\nCategories in index:"
jq -r '.categories | keys[]' "${INDEX_FILE}" | sed 's/^/   /' | sort

# Find missing from index
echo -e "\n${CYAN}2. Missing Categories${NC}"
DISK_CATS=($(ls "${PROMPTS_DIR}/EN/" | sed 's/.md//' | sort))
INDEX_CATS=($(jq -r '.categories | keys[]' "${INDEX_FILE}" | sort))

MISSING=()
for cat in "${DISK_CATS[@]}"; do
    if [[ ! " ${INDEX_CATS[@]} " =~ " ${cat} " ]]; then
        MISSING+=("$cat")
        echo -e "${RED}✗${NC} ${cat} - NOT IN INDEX"
    fi
done

echo -e "\n${CYAN}3. Empty Categories${NC}"
jq -r '.categories | to_entries[] | select(.value.prompts | length == 0) | .key' "${INDEX_FILE}" | while read cat; do
    echo -e "${YELLOW}⚠${NC} ${cat} - 0 prompts"
done

# Generate stats
echo -e "\n${CYAN}4. Statistics${NC}"
DISK_COUNT=${#DISK_CATS[@]}
INDEX_COUNT=${#INDEX_CATS[@]}
MISSING_COUNT=${#MISSING[@]}
INDEXED_PROMPTS=$(jq '[.categories[] | .prompts[]?] | length' "${INDEX_FILE}")

echo "Categories on disk: $DISK_COUNT"
echo "Categories indexed: $INDEX_COUNT"
echo "Missing from index: $MISSING_COUNT ($(( MISSING_COUNT * 100 / DISK_COUNT ))%)"
echo "Total indexed prompts: $INDEXED_PROMPTS"

# Final report location
echo -e "\n${BOLD}${GREEN}✓ Detailed report generated:${NC} ${REPORT_FILE}\n"
